        ...
        if df_frequencia is None or df_frequencia.empty:
            ...
            if 'total_presencas' not in df_temp.columns:
                ...
                df_temp['total_presencas'] = df_temp.apply(
                    lambda row: sum(
                        1 for item in row[freq_cols] if str(item).strip().upper() in ('AUSENTE', 'PRESENTE')),
                    axis=1
                )
...
def formatar_resultados_para_exibicao(resultados):
    ...
    # O get_todos_resultados já deve retornar ordenado
...
def consultar_porcentagens():
    """Função principal chamada pelo Menu (agora mais simples, pois a lógica de cálculo foi movida)"""
    try:
        # Nota: calcular_porcentagens_por_semestre agora é chamado diretamente nos botões para garantir dados recentes.
...
def get_resultados_dashboard():
    """
    NOVA FUNÇÃO: Retorna os resultados dos semestres como uma lista
    de dicionários para o dashboard.
    """
    resultados_db = db.get_todos_resultados()
    resultados_list = []

    for res in resultados_db:
        # Colunas (SQLite): id(0), semestre(1), perc_formados(2), nomes_formados(3), perc_desistentes(4), nomes_desistentes(5), perc_nunca_foram(6), nomes_nunca_foram(7), perc_multisemestre(8), nomes_multisemestre(9)
        resultados_list.append({
            'semestre': res[1],
            'perc_formados': res[2],
            'perc_desistentes': res[4],
            'perc_nunca_foram': res[6]
        })

    return resultados_list


def consultar_dados(resultados_db):
    """Função principal chamada pelo Menu"""
    try:
        resultado_dados_db = db.get_dados_resultados()

        if not resultado_dados_db:
            return "Nenhum dado de semestre encontrado ou processado. Verifique o arquivo principal ('dados_alunos_individuais.xlsx') na pasta 'Dados'."
